...
var UAAClient = (function() {
	...
	UAAClient.prototype.checkAuth = function(res, cb) {
		if(process.env.NODE_ENV === "dev") { cb(true); }

		this.getInfo(function (err, response, data) {

			this.authorized = (data.error || err) ? false : true;

			if(!this.authorized && this.refresh_token) {
				this.refreshAccessToken(res, function (err, response, body) {
